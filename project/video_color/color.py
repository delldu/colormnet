...
class ColorMNet(nn.Module):
    ...
    def encode_value(self, frame, f16, h16, masks): 
        ...
        g16, h16 = self.value_encoder(frame, f16, h16, masks, others)
        return g16, h16
    ...
    def decode_color(self, multi_scale_features, color_feature, hidden_state):
        ...
        hidden_state, logits = self.decoder(*multi_scale_features, hidden_state, color_feature)
...
class ValueEncoder(nn.Module):
    ...
    def forward(self, image, f16, h16, masks, others): # xxxx_gggg
        ...
        # tensor [masks] size: [1, 2, 560, 896], min: -4.593725, max: 4.704186, mean: -0.002239
        # tensor [others] size: [1, 2, 560, 896], min: -4.593725, max: 4.704186, mean: -0.002239

        g = torch.cat([masks, others], dim=0)
        B, C, H, W = g.size()
...
def downsample_groups(g, ratio=1/2, mode='area', align_corners=None):
    # return interpolate_groups(g, ratio, mode, align_corners)
    return F.interpolate(g, scale_factor=0.5, mode='area', align_corners=None)
